CREATE FUNCTION createdyspdlakkwnodrozmiarowkaruchy(tr_kkwnod, integer, boolean) RETURNS integer
    LANGUAGE plpgsql
    AS $_$
DECLARE
 kkwnod          ALIAS FOR $1;
...
BEGIN
 IF (_isRozmiarowka=FALSE) THEN
  RAISE LOG 'DEBUG: createDyspDlaKKWNodRozmiarowkaRuchy(%,%,%): Warunek I', kkwnod.kwe_idelemu, _p_idpracownika, _isRozmiarowka;
  RETURN 0;
 END IF;
 
 IF ((kkwnod.the_flaga&(1<<0))=0 OR kkwnod.kwe_iloscwyk=0) THEN
  RAISE LOG 'DEBUG: createDyspDlaKKWNodRozmiarowkaRuchy(%,%,%): Warunek II (%,%)', kkwnod.kwe_idelemu, _p_idpracownika, _isRozmiarowka, kkwnod.the_flaga, kkwnod.kwe_iloscwyk;
  RETURN 0;
 END IF;
  
 IF ((kkwnod.the_flaga&(1<<12))=0) THEN -- Zle ustawienie kkwnoda  
  RAISE LOG 'DEBUG: createDyspDlaKKWNodRozmiarowkaRuchy(%,%,%): Warunek III (%)', kkwnod.kwe_idelemu, _p_idpracownika, _isRozmiarowka, kkwnod.the_flaga;
  RETURN 0;
 END IF;
 ...
 IF (kkwnod.kwe_nodtype=0) THEN -- KKWNWT_NORMALNOD
  RAISE LOG 'DEBUG: createDyspDlaKKWNodRozmiarowkaRuchy(%,%,%): Operacja - start', kkwnod.kwe_idelemu, _p_idpracownika, _isRozmiarowka;
  ...
  (
   ...
   JOIN tr_ruchy AS r ON (r.knw_idelemusrc=knw.knw_idelemu)
   ...
   JOIN tr_ruchy AS r ON (r.knw_idelemusrc=a.knw_idelemu)
   JOIN tg_transelem AS t ON (r.tel_idelemdst=t.tel_idelem AND a.ttw_idtowaru=t.ttw_idtowaru)
   WHERE a.knw_iloscwykrozm>0
  )
  AS b
  ...
  LOOP    
   ...
   IF (rec_kkwnodwyk.kwc_idruchu>0) THEN
    UPDATE tr_ruchy SET dmag_iddyspozycji=_dmag_iddyspozycji WHERE kwc_idruchu=rec_kkwnodwyk.kwc_idruchu;
   END IF;
   
   RAISE LOG 'DEBUG: createDyspDlaKKWNodRozmiarowkaRuchy(%,%,%): Sukces O (%<->%)', kkwnod.kwe_idelemu, _p_idpracownika, _isRozmiarowka, _knw_idelemu, _dmag_iddyspozycji;
  END LOOP;
  RAISE LOG 'DEBUG: createDyspDlaKKWNodRozmiarowkaRuchy(%,%,%): Operacja - stop', kkwnod.kwe_idelemu, _p_idpracownika, _isRozmiarowka;
  RETURN 1;
 END IF;

CREATE FUNCTION getcyklicznyprzypominacz(date, date) RETURNS SETOF tb_cyklpprow
    LANGUAGE plpgsql
    AS $_$
DECLARE
dataod ALIAS FOR $1;
...
FOR pp IN (SELECT ppm_id, zd_idzdarzenia FROM tb_pp JOIN tb_pracownicyzdarzenia ON (ppm_refid=pzd_idpracownika) JOIN tb_zdarzenia USING (zd_idzdarzenia) WHERE ppm_type = 210 AND ppm_flaga&1=0 AND ((zd_flaga&1536)>>9)=1) 
LOOP
war := TRUE;
lp := 0;
...
IF (date(ret) < dataod) THEN
lp := floor((dataod - date(ret))::int / 7) + 1;
...
SELECT pp.ppm_id, lp, ret INTO rec;
RETURN NEXT rec;
END IF;
ELSE
war := FALSE;

CREATE VIEW kv_raport_bystawka_odetailed AS
 SELECT inn.tr_idtrans,
   ...
   FROM ( SELECT te.tr_idtrans,
            ...
            round(public.vatfrombrt(sum(round((te.tel_wbruttodok + te.tel_wbruttodokkgo), 2)), te.tel_stvat), 2) AS tel_vatb,
            sum(round(public.vatfrombrt(round((te.tel_wbruttodok + te.tel_wbruttodokkgo), 2), te.tel_stvat), 2)) AS tel_vatb2,
            sum(round((te.tel_wbruttodok + te.tel_wbruttodokkgo), 2)) AS tel_brutto,
            ...
            count(*) AS l_pozycji
           FROM kv_raport_bystawka_oinner te
